...
def mandat_1 ():
    ...
    Can_par_metre = (2*np.pi* 8.851e-12) / np.log(Deq/Dsc)
    ...
    GMR1 = np.sqrt(np.sqrt(GMR01*Distance_entre_les_fils**3))
    ...
    Q_div_const = Can_par_metre*Tension_transportee/np.sqrt(3)/(8*np.pi*8.85e-12)
    ...
    E_max = E_max/100
    print("Voltage max a la surface des fils = ", E_max)
...
def mandat_2():
    ...
    Angle_dephasage = np.arcsin((SIL*L)/np.square(Vll))
    Angle_dephasage = (360*Angle_dephasage)/(2*np.pi)
